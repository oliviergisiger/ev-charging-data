...
from configs import STATION_PROPERTIES, STATIONS_FILE, STATIONS_FILE_COLUMNS
...
def read_current_stations_file():
    if STATIONS_FILE.is_file():
        df = pd.read_csv(STATIONS_FILE, parse_dates=['valid_from', 'valid_until'])
...
def upsert_stations(df_new, valid_until):
    ...
    df_new['lastUpdate'] = df_new.lastUpdate.fillna(date(2000, 1, 1))
...
def save_stations_data(data, timestamp: datetime):
    ...
    df_new.loc[:, 'valid_until'] = date(2099, 12, 31)
    df_upserted = upsert_stations(df_new, timestamp.date)
    df_upserted.to_csv(STATIONS_FILE, index=False)
...
def workflow():
    logging.info('START STATIONS WORKFLOW')
    timestamp = datetime.now()
    data = get_data(STATION_PROPERTIES)
    save_stations_data(data, timestamp)
    logging.info('END STATIONS WORKFLOW')
